...
class aStarT:
    def __init__(self,initStates, wallStates, rowsize, colsize):
        ...
        self.mobile = []#{(j, 0) for j in initStates}

    def run(self, initState, goalState, player):#TODO player or astar
        ...
        while currentNod != None:
            self.mobile.append((currentNod.position[0], currentNod.position[1], currentNod.time))
    ...
    def testTemps(self, node):
        ...
        if (node.position[0], node.position[1], node.time) in self.mobile:
